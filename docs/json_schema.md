  ...
  "impl": {
    ...
    "children": {
      "some_concept": {
        ...
        "related_paths": ["impl.other_concept"],
        "aliases": ["nickname"],
        "children": {}
      }
    }
  ...
  "domain": {
    ...
    "children": {
      "business_rule": {
        ...
        "related_paths": ["impl.some_concept"],
        "children": {}
      }
    }
  }
...
- A graph must include at least one of `impl` or `domain`.
- `impl` is for implementation-backed concepts.
- `domain` is for non-code domain concepts.

## Stable paths

If a child appears as `impl.children.views.children.merge_view`, the browser path becomes `impl.views.merge_view`.
...
- `impl` concepts may use implementation-facing metadata such as `loc`, `exploration_coverage`, and `summary_confidence`.
- `impl` concepts may use `implemented` to record whether the represented implementation currently exists.
...
- `/create` creates a new concept under an existing parent. New `impl` concepts default to `implemented: false` unless explicitly overridden.
...
- `/anchor` adds or refines impl-only `loc`, `exploration_coverage`, and related summary metadata.
...
- `impl` should use implementation-oriented kinds such as `module`, `view`, `layout`, `region`, `control`, `behavior`, `transition`, `dataclass`, and `data_group`.
