...
The core idea is that a human or LLM can refer to a concept by a stable derived path like `impl.views.merge_view.pending_selection` or `domain.business_rules.refund_policy` instead of relying on vague natural-language descriptions, and can mention those paths directly while composing prompts for an agent.
...
- Restrict `loc`, `exploration_coverage`, and `summary_confidence` to `impl` concepts; do not add them to `domain` concepts.
