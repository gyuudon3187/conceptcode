...
type ConceptNode = {
  ...
  loc: { file: string; startLine: number; endLine: number } | null
  childPaths: string[]
}

type InspectorKind = "snippet" | "subtree" | "metadata"
...
type SnippetDeps<TState extends SnippetState> = {
  sourcePathForNode: (jsonPath: string, node: ConceptNode) => string | null
  sourceLinesForNode: (sourceFileCache: Map<string, string[]>, jsonPath: string, node: ConceptNode) => string[] | null
}
...
function attributesForToken(token: ThemedToken): number {
  ...
  if (fontStyle & 1) attributes |= TextAttributes.ITALIC
  if (fontStyle & 2) attributes |= TextAttributes.BOLD
  if (fontStyle & 4) attributes |= TextAttributes.UNDERLINE
  return attributes
}
...
export type InspectorPreviewProvider<TState extends SnippetState> = {
  titleFor: (state: TState, node: ConceptNode, kind: InspectorKind) => string
  previewFor: (state: TState, node: ConceptNode, kind: InspectorKind) => Promise<ContextPreview>
...
function treeLine(label: string, indent: string, connector: string, color: RGBA, bold = false): SnippetLine {
  return { chunks: [...(indent ? [createChunk(indent, TREE_CONNECTOR_FG)] : []), ...(connector ? [createChunk(connector, TREE_CONNECTOR_FG)] : []), createChunk(label, color, bold ? TextAttributes.BOLD : 0)] }
}
...
function appendTreeLines<TState extends SnippetState>(lines: SnippetLine[], state: TState, path: string, prefix: string, isLast: boolean, depth: number, maxDepth: number): number {
  const node = state.nodes.get(path)
  if (!node) return 0
  const connector = depth === 0 ? "" : isLast ? "└─ " : "├─ "
  lines.push(treeLine(node.title, prefix, connector, colorForKind(node.kind), depth === 0))
  if (depth === 0 && node.childPaths.length > 0) lines.push(spacerLine())
  if (depth >= maxDepth) {
...
function legendItemsForLines<TState extends SnippetState>(state: TState, node: ConceptNode): PreviewLegendItem[] {
  ...
  while (stack.length > 0) {
    ...
    for (let index = current.childPaths.length - 1; index >= 0; index -= 1) stack.push(current.childPaths[index]!)
  }
  return items.sort((left, right) => left.kindLabel.localeCompare(right.kindLabel))
}

export async function buildContextPreview<TState extends SnippetState>(state: TState, node: ConceptNode, deps: SnippetDeps<TState>): Promise<ContextPreview> {
  if (node.childPaths.length > 0) return buildSubtreePreview(state, node)
  return buildSnippetPreview(state, node, deps)
}

export async function buildSubtreePreview<TState extends SnippetState>(state: TState, node: ConceptNode): Promise<ContextPreview> {
  if (node.childPaths.length > 0) {
    ...
    return { title: `Concept Tree ${node.title}`, lines, legendItems: legendItemsForLines(state, node), useSyntaxStyle: false }
  }
  return { title: `Subtree ${node.title}`, lines: [{ chunks: [createChunk("This concept does not have child concepts.", MUTED_FG)] }], useSyntaxStyle: false }
}

export async function buildMetadataPreview<TState extends SnippetState>(_state: TState, node: ConceptNode): Promise<ContextPreview> {
  const lines: SnippetLine[] = [
  ...
  if (metadataEntries.length > 0) {
    ...
    for (const [key, value] of metadataEntries) lines.push({ chunks: [createChunk(`- ${key}: ${JSON.stringify(value)}`, DEFAULT_CODE_FG)] })
  }
  return { title: `Metadata ${node.title}`, lines, useSyntaxStyle: false }
}

export async function buildSnippetPreview<TState extends SnippetState>(state: TState, node: ConceptNode, deps: SnippetDeps<TState>): Promise<ContextPreview> {
  if (!node.loc) return { title: "Context", lines: [{ chunks: [createChunk("No source preview for this concept.", MUTED_FG)] }], useSyntaxStyle: true }
  const sourcePath = deps.sourcePathForNode(state.jsonPath, node)
  if (!sourcePath) return { title: "Context", lines: [{ chunks: [createChunk("Source location is missing a file path.", ERROR_FG)] }], useSyntaxStyle: true }
  let sourceLines: string[] | null = null
  try {
    sourceLines = deps.sourceLinesForNode(state.sourceFileCache, state.jsonPath, node)
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error)
    return { title: `Context ${node.loc.file}:${node.loc.startLine}-${node.loc.endLine}`, lines: [{ chunks: [createChunk(`Unable to load source: ${message}`, ERROR_FG)] }], useSyntaxStyle: true }
  }
  ...
  const tokenResult = await codeToTokens(snippetText, { lang: language, theme: SHIKI_THEME }).catch(() => null as TokensResult | null)
  const renderedLines = snippetLines.map((lineText, index) => {
    ...
    if (!tokenLine) return { chunks: [...numberedLineChunks(String(lineNumber), lineNumberWidth), createChunk(lineText, DEFAULT_CODE_FG)] }
    return { chunks: [...numberedLineChunks(String(lineNumber), lineNumberWidth), ...(tokenLine.length > 0 ? tokenLine.map(chunkForToken) : [createChunk("", DEFAULT_CODE_FG)])] }
  })
  return { title: `Context ${node.loc.file}:${node.loc.startLine}-${node.loc.endLine}`, lines: renderedLines, useSyntaxStyle: true }
}

export function createConceptCodeInspectorPreviewProvider<TState extends SnippetState>(deps: SnippetDeps<TState>): InspectorPreviewProvider<TState> {
  return {
    titleFor(_state, node, kind) {
      if (kind === "snippet") return node.loc ? `Snippet ${node.loc.file}:${node.loc.startLine}-${node.loc.endLine}` : "Snippet"
      if (kind === "subtree") return `Subtree ${node.title}`
      return `Metadata ${node.title}`
    },
    previewFor(state, node, kind) {
      if (kind === "snippet") return buildSnippetPreview(state, node, deps)
      if (kind === "subtree") return buildSubtreePreview(state, node)
      return buildMetadataPreview(state, node)
    },
    legendItemsFor(preview) {
      return (preview.legendItems ?? []).map((item) => ({ label: item.kindLabel, color: item.color }))
    },
  }
}
