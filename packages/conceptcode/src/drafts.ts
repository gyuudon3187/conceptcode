...
import { clampCursor, currentPath } from "./state"
import type { ConceptGraphState, ConceptNode, CreateConceptDraft, KindDefinition } from "./types"
...
type DraftState = ConceptGraphState & {
  createConceptModal: DraftModalState | null
  confirmModal: {
    kind: string
    title: string
    message: string[]
...
function uniqueChildPath(state: DraftState, parentPath: string, title: string): string {
...
export function isDraftConcept(state: Pick<DraftState, "nodes">, path: string): boolean {
  return Boolean(state.nodes.get(path)?.isDraft)
}

function insertDraftConcept(state: DraftState, draft: CreateConceptDraft, kindDefinition: KindDefinition | null): void {
...
export function removeDraftConcept(state: DraftState, path: string): void {
...
function createKindOptions(state: DraftState, query: string): KindDefinition[] {
...
function exactKindMatch(state: DraftState, query: string): KindDefinition | null {
  const normalizedQuery = query.trim().toLowerCase()
  if (!normalizedQuery) return null
  return state.kindDefinitions.find((item) => item.kind.toLowerCase() === normalizedQuery) ?? null
}

export function openCreateConceptModal(state: DraftState): void {
...
function closeCreateConceptModal(state: DraftState): void {
  state.createConceptModal = null
}

function submitCreateConceptModal(state: DraftState, deps: CreateConceptModalDeps): boolean {
...
export function handleCreateConceptModalKey(state: DraftState, key: KeyEvent, deps: CreateConceptModalDeps): boolean {
  const modal = state.createConceptModal
  if (!modal) return false
...
export function promptToRemoveDraft(state: DraftState, path: string): void {
  state.confirmModal = {
    kind: "remove-draft",
    title: "Remove Draft",
