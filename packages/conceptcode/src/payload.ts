...
type UiMode = "plan" | "build" | "conceptualize"
type ConceptNode = {
  path: string
  title: string
  kind: string | null
...
type PayloadState = {
  uiMode: UiMode
  graphPayload: { interpretation_hint?: Record<string, JsonValue> }
  nodes: Map<string, ConceptNode>
  kindDefinitions: Array<{ kind: string; description: string }>
  projectFiles: string[]
  projectDirectories: string[]
}
type PayloadDeps<TState extends PayloadState> = {
  activeSession: (state: TState) => { draftPromptText: string }
}
...
function renderClipboardBlock(node: ConceptNode): string {
  ...
  if (node.isDraft) lines.push("- draft_status: created in the TUI and not yet present in the source concept graph")
  return `${lines.join("\n")}\n`
}
...
async function referencedFileEntries<TState extends PayloadState>(state: TState, deps: PayloadDeps<TState>): Promise<Array<{ path: string; kind: "file" | "directory"; block: string }>> {
  const paths = referencedFilePaths(deps.activeSession(state).draftPromptText.trim()).filter((path) => state.projectFiles.includes(path) || state.projectDirectories.includes(path)).sort((left, right) => left.localeCompare(right))
  return Promise.all(paths.map(async (path) => {
    if (state.projectDirectories.includes(path)) return { path, kind: "directory" as const, block: renderDirectoryReferenceBlock(path) }
...
function flattenInterpretationHints(value: JsonValue, prefix = ""): string[] {
  ...
  if (value && typeof value === "object") {
    const lines: string[] = []
    for (const [key, nested] of Object.entries(value)) lines.push(...flattenInterpretationHints(nested, prefix ? `${prefix}.${key}` : key))
    return lines
  }
  return []
}

function kindDefinitionsHint<TState extends PayloadState>(state: TState): Record<string, JsonValue> {
  return Object.fromEntries(state.kindDefinitions.filter((definition) => definition.description.trim()).map((definition) => [definition.kind, definition.description]))
}

function normalizedInterpretationHint<TState extends PayloadState>(state: TState): Record<string, JsonValue> {
  const interpretationHint = asMetadataObject(state.graphPayload.interpretation_hint)
  const nextHint: Record<string, JsonValue> = Object.fromEntries(Object.entries(interpretationHint).filter(([key]) => key !== "kind_definitions"))
  const kinds = kindDefinitionsHint(state)
  if (Object.keys(kinds).length > 0) nextHint.kind_definitions = kinds
  return nextHint
}

export async function buildEffectivePrompt<TState extends PayloadState>(state: TState, deps: PayloadDeps<TState>, _currentPath: string): Promise<string> {
  const promptText = deps.activeSession(state).draftPromptText.trim()
  const conceptPaths = referencedConceptPaths(promptText, state.nodes)
  const fileEntries = await referencedFileEntries(state, deps)
  const concepts = conceptPaths.map((path) => renderClipboardBlockWithContext(state.nodes.get(path)!)).join("\n")
  ...
  if (promptText) sections.push(["# Main Instructions", promptText].join("\n\n"))
  if (conceptPaths.length > 0) sections.push("# Concepts", concepts.trimEnd())
  if (fileEntries.length > 0) sections.push("# Files", fileEntries.map((entry) => entry.block).join("\n").trimEnd())
  if (hintLines.length > 0) sections.push(["# Shared Interpretation Hints", ...hintLines].join("\n"))
  return `${sections.join("\n\n")}\n`
}

export async function effectivePromptTokenBreakdown<TState extends PayloadState>(state: TState, deps: PayloadDeps<TState>, _currentPath: string): Promise<EffectivePromptTokenBreakdown> {
  const promptText = deps.activeSession(state).draftPromptText.trim()
  const conceptPaths = referencedConceptPaths(promptText, state.nodes)
  const fileEntries = await referencedFileEntries(state, deps)
  const interpretationHint = normalizedInterpretationHint(state)
  ...
  if (hintLines.length > 0) staticSections.push(["# Shared Interpretation Hints", ...hintLines].join("\n"))
  ...
  const conceptsSection = conceptPaths.length > 0 ? `${["# Concepts", conceptPaths.map((path) => renderClipboardBlockWithContext(state.nodes.get(path)!)).join("\n")].join("\n\n")}\n` : ""
  const filesSection = fileEntries.length > 0 ? `${["# Files", fileEntries.map((entry) => entry.block).join("\n")].join("\n\n")}\n` : ""
...
export async function countEffectivePromptTokens<TState extends PayloadState>(state: TState, deps: PayloadDeps<TState>, currentPath: string): Promise<number> {
  return TOKEN_ENCODING.encode(await buildEffectivePrompt(state, deps, currentPath)).length
}

export async function buildClipboardPayload<TState extends PayloadState>(state: TState, deps: PayloadDeps<TState>, currentPath: string): Promise<string> {
  return buildEffectivePrompt(state, deps, currentPath)
}

export function clipboardSelection<TState extends PayloadState>(state: TState, deps: PayloadDeps<TState>, _currentPath: string): { paths: string[]; count: number } {
  const promptText = deps.activeSession(state).draftPromptText.trim()
